  ...
  const sidebarHTML = `
    ...
    <aside id="app-sidebar" class="fixed top-0 bottom-0 left-0 w-[17.5rem] bg-slate-950/95 backdrop-blur-xl border-r border-slate-800/80 z-50 flex flex-col justify-between -translate-x-full lg:translate-x-0 shadow-2xl">
      ...
      <div id="app-sidebar-nav" class="flex-1 overflow-y-auto px-3 py-4 space-y-6">
        ...
        <!-- Grupo: Configurações & Conexões -->
        <div>
          <div class="sidebar-section-title px-3 mb-2 text-[11px] font-bold text-slate-300 uppercase tracking-wider">
            Configurações
          </div>
          <nav class="space-y-1">
            <a href="/conexoes" data-tooltip="Central de Conexões & APIs" class="sidebar-nav-item flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold transition-all ${
              currentPath === '/conexoes'
                ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 shadow-sm shadow-cyan-500/10'
                : 'text-slate-400 hover:text-slate-100 hover:bg-slate-900/60'
            }">
              <div class="flex items-center gap-3 truncate">
                <i data-lucide="plug-2" class="w-4 h-4 shrink-0 ${currentPath === '/conexoes' ? 'text-cyan-400' : 'text-slate-400'}"></i>
                <span class="sidebar-text truncate">Conexões & APIs</span>
              </div>
              <span class="sidebar-badge px-1.5 py-0.5 rounded text-[10px] font-bold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">Meta Token</span>
            </a>
          </nav>
        </div>
    ...
    <div id="modal-status-api" class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 hidden">
      <div class="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in duration-200">
        ...
        <div class="pt-2 flex items-center justify-between gap-3 border-t border-slate-800">
          <a href="/conexoes" class="text-xs font-semibold text-cyan-400 hover:text-cyan-300 flex items-center gap-1.5 transition-colors">
            <i data-lucide="plug-2" class="w-3.5 h-3.5"></i>
            <span>Gerenciar Conexões & Tokens</span>
          </a>
